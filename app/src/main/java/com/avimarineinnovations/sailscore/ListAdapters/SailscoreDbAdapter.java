...
public class SailscoreDbAdapter {
   ...
   */
  public long createEntry(String boatName, String helm, String crew, String boatclass, int py,
      double orcTotInshore,
      double orcTotOffshore, String sailnumber, String club) {
    ContentValues initialValues = new ContentValues();
    initialValues.put(KEY_NAME, boatName);
  ...
  public Cursor fetchAllEntries() {
    return mDb.query(ENTRIES_TABLE,
        new String[]{KEY_ROWID, KEY_NAME, KEY_HELM, KEY_CREW, KEY_CLASS, KEY_PY, KEY_ORC_INSHORE,
            KEY_ORC_OFFSHORE, KEY_SAILNO, KEY_CLUB}, null, null, null, null, null);
  }
  ...
  public Cursor fetchEntry(long rowId) throws SQLException {
    Cursor mCursor = mDb.query(true, ENTRIES_TABLE, new String[]
            {KEY_ROWID, KEY_NAME, KEY_HELM, KEY_CREW, KEY_CLASS, KEY_PY, KEY_ORC_INSHORE, KEY_ORC_OFFSHORE,
                KEY_SAILNO, KEY_CLUB},
   ...
   */
  public boolean updateEntry(long rowId, String yachtName, String helm, String crew,
      String boatclass, int py,
  ...
  public Cursor fetchAllSeries() {
    return mDb.query(SERIES_TABLE,
        new String[]{KEY_ROWID, KEY_SERIES, KEY_DISCARD_PROFILE, KEY_HCAP, KEY_NUMRACES}, null,
        null, null, null, null);
  }
  ...
  public boolean updateSeries(long rowId, String series_name, int numRaces, String discardProfile,
      int hcap) {
  ...
  public boolean updateResult(Long entry, Long series, Long race, int result, int start, int finish,
      int laps, int totalLaps, int resultCode, int rdg_points, float points, int scored,
      int discarded,
    ...
    Cursor mCursor = mDb.query(true, RESULTS_TABLE, new String[]
            {KEY_ROWID, KEY_RACE, KEY_ENTRY, KEY_SERIES, KEY_RESULT, KEY_START_TIME, KEY_FINISH_TIME,
                KEY_LAPS, KEY_TOTAL_LAPS, KEY_CODE, KEY_RDG_POINTS, KEY_POINTS, KEY_SCORED,
                KEY_DISCARDED, KEY_POSITION, KEY_TOTAL, KEY_NETT},
        KEY_ENTRY + " = " + entry + " AND " + KEY_RACE + " = " + race + " AND " + KEY_SERIES + " = "
  ...
  public boolean addResultRow(Long entry, Long series, int race, int result, int start, int finish,
      int laps,
      int totalLaps, int resultCode, int rdg_points, int points, int scored, int discarded,
      int position, int total, int nett) {
    ContentValues args = new ContentValues();
    args.put(KEY_RACE, race);
    args.put(KEY_ENTRY, entry);
    args.put(KEY_SERIES, series);
  ...
  public boolean deleteResultsBySeriesEntry(Long seriesId, String entry) {
    ...
    return mDb.delete(RESULTS_TABLE, KEY_ENTRY + "=" + entry + " AND " + KEY_SERIES + "=" + series,
        null) > 0;
  }
  ...
  public boolean deleteResultByRace(Long seriesId, int raceNo) {
    ...
    return
        mDb.delete(RESULTS_TABLE, KEY_SERIES + "=" + series + " AND " + KEY_RACE + "=" + race, null)
            > 0;
  }
  ...
  public Cursor getResults(Long entry, Long series) {
    ...
    Cursor mCursor = mDb.query(true, RESULTS_TABLE, new String[]
            {KEY_ROWID, KEY_RACE, KEY_RESULT, KEY_START_TIME, KEY_FINISH_TIME, KEY_LAPS, KEY_TOTAL_LAPS,
                KEY_CODE, KEY_RDG_POINTS, KEY_POINTS, KEY_SCORED, KEY_DISCARDED, KEY_POSITION,
                KEY_TOTAL, KEY_NETT},
        KEY_ENTRY + "=" + mEntry + " AND " + KEY_SERIES + "=" + mSeries, null, null, null, null,
        null);
    if (mCursor != null) {
      mCursor.moveToFirst();
    }
  ...
  public Cursor fetchEntriesFromSeries(Long seriesId) {
    String series = seriesId.toString();
    final String mQuery =
        "SELECT entries._id, entries.helm, entries.crew, entries.boatclass, entries.sailnumber, entries.club, entries.py "
  ...
  public Cursor fetchRaceByNumber(int race, Long seriesId) {
    ...
    final String mQuery =
        "SELECT results._id, results.entry, results.start_time, results.finish_time, " +
    ...
    Cursor mCursor = mDb
        .rawQuery(mQuery, new String[]{String.valueOf(sRace), String.valueOf(sSeries)});
    if (mCursor != null) {
      mCursor.moveToFirst();
    }
    return mCursor;
