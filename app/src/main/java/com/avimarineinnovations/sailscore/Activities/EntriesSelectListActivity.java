...
public class EntriesSelectListActivity extends ListActivity {
    ...
    mConfirmSelectionButton.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        int numRaces = seriesCursor
            .getInt(seriesCursor.getColumnIndexOrThrow(SailscoreDbAdapter.KEY_NUMRACES));
        seriesCursor.close();
        ...
        if (entriesSelected.length == 0) {
          ...
          boolean[] entriesDeselected = getDeselected(
              entriesSelected); // bitmap of old entries to delete
  ...
  private boolean[] getListOfEntries(Long series) {
    ...
    int index = 0;
    if (entriesCursor != null && entriesCursor
        .moveToFirst()) { // moveToFirst returns false if cursor is empty
      do {
        //Find the id of each entry
        int entryId = entriesCursor
            .getInt(entriesCursor.getColumnIndex(SailscoreDbAdapter.KEY_ROWID));
        // Now see if the entryId matches one of those in the selectionCursor
        if (selectionCursor != null && selectionCursor.moveToFirst()) {
          do {
            int currentId = selectionCursor
                .getInt(selectionCursor.getColumnIndex(SailscoreDbAdapter.KEY_ROWID));
  ...
  private void insertResultRows(boolean[] entriesSelected, int numRaces) {
    ...
    for (int entry = 0; entry < entriesSelected.length; entry++) {
      if (entriesSelected[entry] == true) {
        Long entryId = entryCursor
            .getLong(entryCursor.getColumnIndex(SailscoreDbAdapter.KEY_ROWID));
