...
import com.avimarineinnovations.sailscore.Objects.EntryTimesObj;
import com.avimarineinnovations.sailscore.ListAdapters.RaceTimesEntryListAdapter;
...
public class RaceTimesEntryListActivity extends ListActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    mSaveResultsButton = findViewById(R.id.confirm_results_button);
    mRaceText = findViewById(R.id.race_times_id);
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    ...
    mi.inflate(R.menu.times_for_race_menu, menu);
    return true;
  }
    ...
    mSaveResultsButton.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        for (int i = 0; i < startTimes.length; i++) {
          Long entryId = entriesCursor
              .getLong(entriesCursor.getColumnIndex(SailscoreDbAdapter.KEY_ROWID));
          ...
          mDbHelper.updateResult(entryId, mRowId, raceId, 0, startTimes[i], finishTimes[i],
              lapsSailed[i], totalLaps[i], resultCodes[i], redress[i], 0, 0, 0, 0, 0, 0);
  ...
  private void fillData() {
    ...
    if (raceCursor != null && raceCursor.moveToFirst()) {
      ...
      for (int i = 0; i < raceCursor.getCount(); i++) {
        ...
        String boatClass = raceCursor
            .getString(raceCursor.getColumnIndex(SailscoreDbAdapter.KEY_CLASS));
        String sailNo = raceCursor
            .getString(raceCursor.getColumnIndex(SailscoreDbAdapter.KEY_SAILNO));
        String competitor = helm + "\n" + crew + "\n" + boatClass + " " + sailNo;
        combinedObj.setCompetitor(competitor);
        // Try this method instead of the commented out one
        String sStartTime = raceCursor
            .getString(raceCursor.getColumnIndex(SailscoreDbAdapter.KEY_START_TIME));
        String sFinishTime = raceCursor
            .getString(raceCursor.getColumnIndex(SailscoreDbAdapter.KEY_FINISH_TIME));
        ...
        int redressPosition = raceCursor
            .getInt(raceCursor.getColumnIndex(SailscoreDbAdapter.KEY_RDG_POINTS));
        int sMins = sSecs / 60;
        sSecs %= 60;
        int fMins = fSecs / 60;
        fSecs %= 60;
        int lapsSailed = Integer
            .parseInt(raceCursor.getString(raceCursor.getColumnIndex(SailscoreDbAdapter.KEY_LAPS)));
        int totalLaps = Integer.parseInt(
            raceCursor.getString(raceCursor.getColumnIndex(SailscoreDbAdapter.KEY_TOTAL_LAPS)));
  ...
  private int[] getEnteredLaps() {
    ...
    for (int i = 0; i < resultCount; i++) {
      try {
        laps[i] = laps.equals("") ? 0
            : Integer.parseInt(RaceTimesEntryListAdapter.combinedList.get(i).getLaps());
      } catch (NumberFormatException e) {
  ...
  private int[] getEnteredTotals() {
    ...
    for (int i = 0; i < resultCount; i++) {
      try {
        laps[i] = laps.equals("") ? 0
            : Integer.parseInt(RaceTimesEntryListAdapter.combinedList.get(i).getTotalLaps());
      } catch (NumberFormatException e) {
        laps[i] = 0; // for now just put something in that won't break
      }
    }
  ...
  private int[] getEnteredRedress() {
    ...
    for (int i = 0; i < resultCount; i++) {
      try {
        positions[i] = positions.equals("") ? 0
            : Integer.parseInt(RaceTimesEntryListAdapter.combinedList.get(i).getRedressPosition());
      } catch (NumberFormatException e) {
