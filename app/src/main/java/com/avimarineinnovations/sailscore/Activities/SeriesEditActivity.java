...
public class SeriesEditActivity extends Activity {
  ...
  private RadioButton mOrcInshoreButton;
  private RadioButton mOrcOffshoreButton;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    mOrcInshoreButton = (RadioButton) findViewById(R.id.radioOrcTotInShore);
    mOrcOffshoreButton = (RadioButton) findViewById(R.id.radioOrcTotOffShore);
    ...
    mSaveButton.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        ...
        switch (saveState()) {
          ...
          default:
            setResult(RESULT_OK);
            Toast.makeText(SeriesEditActivity.this, getString(R.string.series_saved_message),
  ...
  private int saveState() {
    ...
    if (selected==R.id.radioHcap)
      hcap = HandicapType.PY.ordinal();
    else if (selected==R.id.radioOrcTotInShore)
      hcap = HandicapType.ORC_TOT_INSHORE.ordinal();
    else if (selected==R.id.radioOrcTotOffShore)
      hcap = HandicapType.ORC_TOT_OFFSHORE.ordinal();
    else if (selected==R.id.radioFleet)
      hcap = HandicapType.FLEET.ordinal();
    if (mRowId == null) {
      ...
      int oldNumRaces = Integer.parseInt(
          seriesFromDb.getString(seriesFromDb.getColumnIndex(SailscoreDbAdapter.KEY_NUMRACES)));
      seriesFromDb.close();
      ...
      if (oldNumRaces > numRaces) { // we have races to delete
        ...
        for (int entry = 1; entry <= numEntries; entry++) {
          Long entryId = entriesFromDb
              .getLong(entriesFromDb.getColumnIndex(SailscoreDbAdapter.KEY_ROWID));
  ...
  private void populateFields() {
    if (mRowId != null && mRowId != 0) {
      ...
      HandicapType hcap = HandicapType.values()[series.getInt(series.getColumnIndex(SailscoreDbAdapter.KEY_HCAP))];
      if (hcap == HandicapType.FLEET) {
        mFleetButton.setChecked(true);
        mHcapButton.setChecked(false);
        mOrcInshoreButton.setChecked(false);
        mOrcOffshoreButton.setChecked(false);
      } else if (hcap == HandicapType.PY) {
        mFleetButton.setChecked(false);
        mHcapButton.setChecked(true);
        mOrcInshoreButton.setChecked(false);
        mOrcOffshoreButton.setChecked(false);
      } else if (hcap == HandicapType.ORC_TOT_INSHORE) {
        mFleetButton.setChecked(false);
        mHcapButton.setChecked(false);
        mOrcInshoreButton.setChecked(true);
        mOrcOffshoreButton.setChecked(false);
      }else {
        mFleetButton.setChecked(false);
        mHcapButton.setChecked(false);
        mOrcInshoreButton.setChecked(false);
        mOrcOffshoreButton.setChecked(true);
      }
      series.close();
    }
  }
