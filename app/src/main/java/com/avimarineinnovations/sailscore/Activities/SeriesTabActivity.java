...
import com.avimarineinnovations.sailscore.Fragments.SeriesEntriesListFragment;
import com.avimarineinnovations.sailscore.Fragments.SeriesRacesListFragment;
...
public class SeriesTabActivity extends Activity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    tabA.setTabListener(
        new TabListener<SeriesEntriesListFragment>(this, "Tag A", SeriesEntriesListFragment.class));
    actionBar.addTab(tabA);

    Tab tabB = actionBar.newTab();
    tabB.setText(getString(R.string.enter_by_race));
    tabB.setTabListener(
        new TabListener<SeriesRacesListFragment>(this, "Tag B", SeriesRacesListFragment.class));
    ...
    switch (item.getItemId()) {
      ...
      case R.id.menu_delete:
        ...
        builder1.setMessage(R.string.confirm_delete_message)
            ...
            .setNegativeButton(R.string.cancel_delete_series,
  ...
  private String makeCsv(Long series, boolean raw) {
    ...
    int numRaces = seriesCursor
        .getInt(seriesCursor.getColumnIndex(SailscoreDbAdapter.KEY_NUMRACES));
    ...
    for (int line = 0; line < numEntries; line++) {
      ...
      String position = resultsCursor
          .getString(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_POSITION));
      String helm = entriesCursor
          .getString(entriesCursor.getColumnIndex(SailscoreDbAdapter.KEY_HELM));
      String crew = entriesCursor
          .getString(entriesCursor.getColumnIndex(SailscoreDbAdapter.KEY_CREW));
      String club = entriesCursor
          .getString(entriesCursor.getColumnIndex(SailscoreDbAdapter.KEY_CLUB));
      String boatclass = entriesCursor
          .getString(entriesCursor.getColumnIndex(SailscoreDbAdapter.KEY_CLASS));
      String sailno = entriesCursor
          .getString(entriesCursor.getColumnIndex(SailscoreDbAdapter.KEY_SAILNO));
      ...
      float total = resultsCursor
          .getFloat(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_TOTAL));
      float nett = resultsCursor
          .getFloat(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_NETT));
      ...
      boolean scored =
          resultsCursor.getString(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_SCORED))
              .equals("1") ? true : false;
      if (!raw) {
        resultString = resultString + position + ", ";
      }
      resultString =
          resultString + helm + ", " + crew + ", " + club + ", " + boatclass + ", " + sailno + ", "
              + py;
      String result;
      float points;
      ...
      for (int race = 0; race < numRaces; race++) {
        ...
        if (seriesType >= 1) { // for handicap series we need time information
          String sStartTime = resultsCursor
              .getString(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_START_TIME));
          String sFinishTime = resultsCursor
              .getString(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_FINISH_TIME));
          ...
          int sailedLaps = resultsCursor
              .getInt(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_LAPS));
          ...
          result = resultsCursor
              .getString(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_RESULT));
        }
        ...
        String rdgPoints = resultsCursor
            .getString(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_RDG_POINTS));
        // If the series has been scored then there will be a value for discarded or 0 if not
        boolean discarded =
            resultsCursor.getInt(resultsCursor.getColumnIndex(SailscoreDbAdapter.KEY_DISCARDED))
                == 1 ? true : false;
        resultString = resultString + ", ";
        ...
        switch (resultCode) {
          ...
          case 11:
            if (raw || !scored) {
              ...
              resultString =
                  resultString + result + " " + mHelper.convertResultCode(resultCode) + "-"
                      + rdgPoints;
            } else {
  ...
  private void sendFile(String fileContents, boolean raw) {
    ...
    Cursor seriesCursor = mDbHelper.fetchSeries(mRowId);
    String seriesName = seriesCursor
